...
object ApplicationFactory extends StrictLogging {
  def tryCreate(rabbitMqClient: Client, name: String, rmqServerName: String, vhost: String, queueName: String, maxMessagesCount: Int, maxInstancesCount: Option[Int] = None, minInstancesCount: Option[Int] = None): Try[Application] = {
    rabbitMqClient.queueExists(vhost, queueName) match {
      case Success(true) =>
        Success(ApplicationImpl(name, rmqServerName, vhost, queueName, maxMessagesCount, maxInstancesCount, minInstancesCount))
      case Failure(e) =>
  ...
  private case class ApplicationImpl(name: String, rmqServerName: String, vhost: String, queueName: String, maxMessagesCount: Int, maxInstancesCount: Option[Int] = None, minInstancesCount: Option[Int] = None) extends Application
}

...
class AutoscalerActor(checkPeriod: FiniteDuration, applicationsReCheckPeriod: FiniteDuration) extends Actor with ActorLogging {
  import context.dispatcher
  val tick =
    context.system.scheduler.schedule(500 millis, applicationsReCheckPeriod, self, CheckNewApplications)

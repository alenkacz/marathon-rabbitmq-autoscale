...
object MarathonProxy extends StrictLogging {
  ...
  private def scale(marathonClient: Marathon,
    ...
    targetCount match {
      case newInstanceCount if targetCount != applicationState.getInstances =>
        ...
        app.setId(applicationName)
        app.setInstances(targetCount)

        marathonClient.updateApp(applicationName, app, true)
      case _ =>
        logger.debug(
          s"Application already have target (min/max) count of instances which is $targetCount")
    }
  }
  ...
  def findAppsWithAutoscaleLabels(
    ...
    val labelledApps = marathonClient.getApps.getApps.asScala
      .filter(a =>
        a.getLabels != null && a.getLabels.asScala.exists(p =>
          p._1.equalsIgnoreCase(QUEUE_LABEL_NAME)) && a.getLabels.asScala
          .exists(p => p._1.equalsIgnoreCase(MAX_MESSAGES_LABEL_NAME)))
      .map(a => {
        val labels = a.getLabels.asScala
        val queueName = labels
          .find(_._1.equalsIgnoreCase(QUEUE_LABEL_NAME))
          .map(_._2.trim)
          .get
        val vhostName = labels
          .find(_._1.equalsIgnoreCase(VHOST_LABEL_NAME))
          .map(_._2.trim)
          .getOrElse("/")
        val maxMessagesCount = labels
          .find(_._1.equalsIgnoreCase(MAX_MESSAGES_LABEL_NAME))
          .map(_._2)
          .get
          .toInt
        val maxInstancesCount = labels
          .find(_._1.equalsIgnoreCase(MAX_INSTANCES_LABEL_NAME))
          .map(_._2)
          .map(_.toInt)
        val minInstancesCount = labels
          .find(_._1.equalsIgnoreCase(MIN_INSTANCES_LABEL_NAME))
          .map(_._2)
          .map(_.toInt)
        val serverName = labels
    ...
    logger.info(
      s"Configured following apps via marathon labels: '${labelledApps.map(a => a).mkString(",")}'")
    labelledApps
  }
}

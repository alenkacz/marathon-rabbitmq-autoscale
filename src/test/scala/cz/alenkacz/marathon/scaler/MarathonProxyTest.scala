...
class MarathonProxyTest extends TestFixture with MockitoSugar {
  ...
  private def appWithLabel(appName: String, queueName: String = "test"): GetAppsResponse = {
    ...
    app.setId(appName)
    app.setLabels(Map(MarathonProxy.QUEUE_LABEL_NAME -> queueName, MarathonProxy.MAX_MESSAGES_LABEL_NAME -> "10", MarathonProxy.MAX_INSTANCES_LABEL_NAME -> "5", MarathonProxy.MIN_INSTANCES_LABEL_NAME -> "0"))
